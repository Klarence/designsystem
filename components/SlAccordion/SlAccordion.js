...
accordionTemplate.innerHTML = `<slot></slot>`;

/**
 * `SlAccordion` is a container element for headings and panels.
 *
 * Each heading must be a `<sl-accordion-heading>`. Each panel must be a
 * `<sl-accordion-panel>` and must be adjacent to a heading.
 */
export class SlAccordion extends HTMLElement {
    constructor() {
        super();
        const shadowRoot = this.attachShadow({mode: 'closed'});
        shadowRoot.adoptedStyleSheets = [sheet];
        shadowRoot.appendChild(accordionTemplate.content.cloneNode(true));
    }
    ...
    connectedCallback() {
        ...
        // `<sl-accordion-headers>` emit a custom event when the heading is
        ...
        // Wait for `<sl-accordion-heading>` and `<sl-accordion-panel`
        // to have booted before proceeding.
        Promise.all([
            customElements.whenDefined('sl-accordion-heading'),
            customElements.whenDefined('sl-accordion-panel'),
        ])
            .then(_ => {
                ...
                headings.forEach(heading => {
                    // All buttons inside the `SlAccordionHeadings` are made
     ...
     * is a `<sl-accordion-heading>`.
     */
    _isHeading(elem) {
        return elem.tagName.toLowerCase() === 'sl-accordion-heading';
    }
     ...
     */
    _allPanels() {
        return Array.from(this.querySelectorAll('sl-accordion-panel'));
    }
     ...
     */
    _allHeadings() {
        return Array.from(this.querySelectorAll('sl-accordion-heading'));
    }
    ...
    _panelForHeading(heading) {
        ...
        if (next.tagName.toLowerCase() !== 'sl-accordion-panel') {
    ...
    _animate(panel, startOffset, endOffset) {
        ...
        // Set the `animating` class on the `<sl-accordion>` element. This
        ...
        // element. Switch the `<sl-accordion>` element to `overflow: hidden`
        // to prevent that.
...
window.customElements.define('sl-accordion', SlAccordion);
...
// `<sl-accordion-heading>` element rather than the button itself.
...
/**
 * `SlAccordionHeading` is the element for the headings in the accordion.
 ...
 *  problematic. This element dispatches a `sl-accordion-change` event when
 ...
 */
class SlAccordionHeading extends HTMLElement {
    ...
    constructor() {
        ...
        // handler will always be the `<sl-accordion-heading>`, even if the
    ...
    connectedCallback() {
        ...
        if (!this.id)
            this.id = `sl-accordion-heading-generated-${headingIdCounter++}`;
...
window.customElements
    .define('sl-accordion-heading', SlAccordionHeading);
...
// ShadyCSS.prepareTemplate(accordionPanelTemplate, 'sl-accordion-panel');
// /HIDE
...
/**
 * `SlAccordionPanel` is the element for the expandable and collapsible
 ...
 */
class SlAccordionPanel extends HTMLElement {
    constructor() {
        super();
    ...
    connectedCallback() {
        ...
        if (!this.id)
            this.id = `sl-accordion-panel-generated-${panelIdCounter++}`;
    }
...
window.customElements.define('sl-accordion-panel', SlAccordionPanel);
